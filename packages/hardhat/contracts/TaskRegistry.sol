	...
	SchemaResolver
{
	...
	string public constant taskCreationSchema =
	...
	string public constant taskFundedSchema =
	...
	string public constant taskApplicationSchema =
		"bytes32 taskUID,bytes32 userUID,bytes32[] skillUIDs";
	bytes32 public immutable taskApplicationSchemaUID;
	string public constant taskStartedSchema =
		"bytes32 taskUID,bytes32 userUID,uint256 startTimestamp";
	bytes32 public immutable taskStartedSchemaUID;
	string public constant taskCompletedSchema =
		"bytes32 taskUID,bytes32 userUID,uint256 completeTimestamp";
	bytes32 public immutable taskCompletedSchemaUID;
	string public constant taskReviewSchema =
		"bytes32 taskUID,bytes32 userUID,string comments";
	...
	constructor(
		...
		taskCreationSchemaUID = _schemaRegistry.register(
			taskCreationSchema,
			this,
			false
		);
		taskFundedSchemaUID = _schemaRegistry.register(
			taskFundedSchema,
			this,
			true
		);
		taskApplicationSchemaUID = _schemaRegistry.register(
			taskApplicationSchema,
			this,
			true
		);
		taskStartedSchemaUID = _schemaRegistry.register(
			taskStartedSchema,
			this,
			false
		);
		taskCompletedSchemaUID = _schemaRegistry.register(
			taskCompletedSchema,
			this,
			false
		);
		taskReviewSchemaUID = _schemaRegistry.register(
			taskReviewSchema,
			this,
			true
		);
	...
	function onAttest(
		...
		if (attestation.schema == taskCreationSchemaUID) {
			...
			require(
				bytes(communityRegistry.communityByUID(communityUID).name)
					.length > 0,
			...
			(
				...
				string memory tokenName
					...
					attestation.data,
					(address, bool, bool, uint, uint, string)
				);
			...
			(bytes32 taskUID, bytes32 userUID, bytes32[] memory skillUIDs) = abi
				.decode(attestation.data, (bytes32, bytes32, bytes32[]));
			require(
				taskStatusByUID[taskUID] == TaskStatus.CREATED,
				"Task is either already in-progress or completed"
			);
			...
			for (uint i; i < skillUIDs.length; ++i) {
				Attestation memory userSkill = _eas.getAttestation(
					skillUIDs[i]
				);
				(bytes32 _skillUID, bytes32 _userUID) = abi.decode(
					userSkill.data,
					(bytes32, bytes32)
				);
				require(
					_skillUID != bytes32(0),
					"User skill attestation not found"
				);
				require(
					_userUID == userUID,
					"User UID does not match skill attestation"
				);
			}
			taskApplicantsByTaskUID[taskUID].push(user.account);
			emit TaskApplicationSubmitted(
				taskUID,
				attestation.uid,
				userUID,
				skillUIDs
			...
			(bytes32 taskUID, bytes32 userUID, ) = abi.decode(
				attestation.data,
				(bytes32, bytes32, uint)
			);
			require(
				attestation.attester == taskByUID(taskUID).creator,
			...
			UserRecord memory user = userRegistry.userRecordByUID(userUID);
			if (user.account != attestation.recipient)
	...
	function payoutTaskRewards(bytes32 taskUID, address to) internal {
		for (uint i; i < taskRewardUIDsByTaskUID[taskUID].length; ++i) {
			...
			Attestation memory rewardAttestation = _eas.getAttestation(
				rewardUID
			);
			(
				address tokenAddress,
