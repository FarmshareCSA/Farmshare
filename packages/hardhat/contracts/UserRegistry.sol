...
contract UserRegistry is IUserRegistry, Ownable, SchemaResolver {
	...
	string public constant updateSchema =
	...
	string public constant userSkillSchema =
		"bytes32 skillUID,bytes32 userUID,string skillName";
	bytes32 public immutable userSkillSchemaUID;
	string public constant skillEndorsementSchema =
		"bytes32 userSkillUID,bytes32 userUID,bytes32 endorserUID";
	...
	mapping(bytes32 => mapping(string => bytes32))
		public userSkillUIDByUserAndSkillName;

	constructor(
		IEAS eas,
		ISchemaRegistry registry
	) SchemaResolver(eas) Ownable() {
		registrationSchemaUID = registry.register(
			registrationSchema,
			this,
			true
		);
		...
		skillEndorsementSchemaUID = registry.register(
	...
	function userRecordByUID(
		...
		(
			...
			UserRole _role
				...
				attestation.data,
				(address, string, bytes32, string, UserRole)
			);
		if (registrationUpdatesByOriginalUID[userUID] != bytes32(0)) {
			...
			(_account, _name, _emailHash, _location, _role) = abi.decode(
	...
	function onAttest(
		...
		if (attestation.schema == registrationSchemaUID) {
			...
			require(
				userRegistrations[_account] == bytes32(0),
				"User already registered"
			);
			require(
				userEmailHashToAddress[_emailHash] == address(0),
			...
			UserRecord memory originalRecord = userRecordByUID(
				attestation.refUID
			);
			require(
				originalRecord.account == attestation.attester,
				"Only original account can update"
			);
			registrationUpdatesByOriginalUID[attestation.refUID] = attestation
				.uid;
			...
			require(
				skillRecordsBySkillName[newSkill] == bytes32(0),
				"Skill already recorded"
			);
			...
			require(
				userSkillUIDByUserAndSkillName[_userUID][skillName] ==
					bytes32(0),
			...
			userSkillUIDByUserAndSkillName[_userUID][skillName] = attestation
				.uid;
			...
			require(
				attestation.refUID == _userSkillUID,
			...
			require(
				endorserRecord.account != address(0),
				"Endorser not registered"
			);
			Attestation memory skillRecord = _eas.getAttestation(_skillUID);
			string memory skillName = abi.decode(skillRecord.data, (string));
			emit UserSkillEndorsed(
				_userUID,
				_endorserUID,
				_skillUID,
				skillName
